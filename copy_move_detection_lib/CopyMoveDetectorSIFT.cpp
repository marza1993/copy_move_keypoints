...
void CopyMoveDetectorSIFT::filtraggioMatchLowe()
{
	...
	for (int id : indici_match_validi)
	{
		...
		if (n_matchForCoordsCouple.count(key) == 0)
		{
			...
			// ------- ok -----------
			cv::Mat riga1 = descriptors.row(match_indici_lowe[id][0]);
			cv::Mat riga2 = descriptors.row(match_indici_lowe[id][1]);
			KeyPointsMatch kpm(kp1, kp2, &riga1, &riga2, descriptorDistancesLowe[id], matchID);

			// ----- errore, non compila! --------
			//KeyPointsMatch kpm(kp1, kp2, &descriptors.row(match_indici_lowe[id][0]), &descriptors.row(match_indici_lowe[id][0]), descriptorDistancesLowe[id], matchID);
